...
<head>
    ...
    $(function(){
    	$("#moveToRight").bind('click',function(){
        	var sl = $("#availableRoles").find("option:selected");
        	var val = sl.val();
        	if (val != "") {
	        	$("#userRoles").append("<option value='" + val + "''>"+val+"</option>");
	        	sl.remove();
	        	$("#hiddenRoleList").append("<input type='hidden' name='userRoles' value='"+val+"'/>");
        	}
        });
        
    	$("#moveToLeft").bind('click',function(){
    		var sl = $("#userRoles").find("option:selected");
        	var val = sl.val();
        	if (val != "") {
	        	$("#availableRoles").append("<option value='" + val + "''>"+val+"</option>");
	        	sl.remove();
	        	var len = $("#hiddenRoleList input").length;
	        	for(var i=0;i<len;i++) {
	        		if ($("#hiddenRoleList input").eq(i).val()==val) {
	        			$("#hiddenRoleList input").eq(i).remove();
	        		}
	        	}
        	}
        });
...
<ul>
    <li class="buttonBar right">
        <input type="submit" class="btn" name="save" onclick="bCancel=false" value="<fmt:message key="button.save"/>"/>
...
<c:choose>
    ...
    <li>
    	<fieldset>
    		<legend><fmt:message key="operator.task"/></legend>
    		<appfuse:label key="department.code"/>
            <form:input path="department" id="department" cssClass="text large"/>
            <appfuse:label key="lab.code"/>
            <form:input path="labCode" id="labCode" cssClass="text large"/>
    ...
    <li>
        <fieldset class="pickList">
            ...
            <table class="pickList">
                ...
                <tr>
				    <td>
				        <select id="availableRoles" name="availableRoles" size="5">
				    ...
				    <td >
				       ...
				       <input type="button" id="moveToLeft" value="<<"/>
				    </td>
				    <td>
				        <select id="userRoles" name="userRoles" size="5">
				        ...
				        <div id="hiddenRoleList">
				        	<c:if test="${rightList != null}">
						        <c:forEach var="list" items="${rightList}" varStatus="status">
						            <input type="hidden" name="userRoles" value="<c:out value="${list.value}"/>"/>
						        </c:forEach>
						    </c:if>
				        </div>
				    </td>
    ...
    <script type="text/javascript">
    	var leftlen = $("#availableRoles option").length;
    	for (var i=0;i<leftlen;i++) {
    		var lo = $("#availableRoles option").eq(i);
    		var rightlen = $("#userRoles option").length;
    		for (var j=0;j<rightlen;j++) {
    			var ro = $("#userRoles option").eq(j);
    			if (lo.val() == ro.val()) {
    				lo.css('display','none');
    			}
    		}
